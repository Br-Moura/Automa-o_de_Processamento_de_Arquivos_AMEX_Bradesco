...
# Dados fixos para o produto: Banco Bradesco AMEX
STR_LAYOUT = 'C:\MATICA\MACHINE\MP_Layouts\BRADESCO_AMEX.mpl'
STR_XML = 'C:\MATICA\MACHINE\MP_Databases\XML\AMEX\AMEX_GD.XML'
STR_BD = 'C:\MATICA\MACHINE\MP_Databases\PRD\BRADESCO_AMEX'
STR_LETTER = ''
STR_TITLE = ' ** AVISO ** ==> SELECIONE A DATA DO MOVIMENTO BANCO BRAD AMEX'
...
def select_file():
    Tk().withdraw()
    initialdir_str = 'U:\PAYMENT\BRD\PRIORITY\MATICA'
    title_str = STR_TITLE
...
def find_prd_file(file_fullname):
    ...
    MC_dir = 'U:\PAYMENT\BRD\PRIORITY\CRD_CJ385_1984' + dir_name[pos_date:]
...
def parse_matica_line(line):
    try:
        ...
        match = re.search(r'(\d{7})\$.*#EMB1#(\d{4} \d{6} \d{5})', line)
        if match:
...
def parse_prd_record(record):
    try:
        # Ajustar regex para capturar número de registro antes de #EMB01# e número do cartão após #EMB05#
        match = re.search(r'(\d{7})\$.*#EMB1#(\d{4} \d{6} \d{5})', record)
        if match:
            register_number = match.group(1)
